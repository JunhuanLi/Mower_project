...
struct stm32_serial_int_rx uart8_int_rx;
struct stm32_serial_device uart8 =
{
	UART8,//UART8_BASE
	&uart8_int_rx,
	RT_NULL
};
struct rt_device uart8_device;
//#endif
...
#ifdef RT_USING_UART8
		/* Configure USART3 Rx/tx PIN */
		GPIO_InitStructure.GPIO_Pin = UART8_GPIO_RX | UART8_GPIO_RX;
		GPIO_Init(UART8_GPIO, &GPIO_InitStructure);

    /* Connect alternate function */
    GPIO_PinAFConfig(UART8_GPIO, UART8_TX_PIN_SOURCE, GPIO_AF_UART8);//((uint8_t)0x08));//GPIO_AF_USART8 = ((uint8_t)0x08)
...
//#ifdef RT_USING_UART8
	...
	/* register uart8 */
	rt_hw_serial_register(&uart8_device, "uart8",
		RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_STREAM,
		&uart8);

	/* Enable USART8 DMA Rx request */
