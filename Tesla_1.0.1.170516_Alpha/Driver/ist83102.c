...
/*******************************************************************************
  Function		: ist8310_iic2_write_register     
...
T_bool mag_init2(void)
{
	...
	iic2_init();	
  delay_ms(50);
	ist8310_iic2_write_register(MAG_SLAVE_ADDRESS, 0x41, 0x24);
	ist8310_iic2_write_register(MAG_SLAVE_ADDRESS, 0x42, 0xC0);
	
//	ist8310_iic2_read_register(LEFT_MAG_I2C, MAG_SLAVE_ADDRESS, WHO_AM_I, &id);
...
static void update_mag(void)
{
	...
	ist8310_iic2_write_register(MAG_SLAVE_ADDRESS, 0x0A, 0x01);

  delay_ms(6);
	ist8310_iic2_read_register(MAG_SLAVE_ADDRESS, MAG_X_L, &data[0]);
	ist8310_iic2_read_register(MAG_SLAVE_ADDRESS, MAG_X_H, &data[1]);
	g_mag.mag_x = (s16)(data[1] << 8 | data[0]);

	ist8310_iic2_read_register(MAG_SLAVE_ADDRESS, MAG_Y_L, &data[0]);
	ist8310_iic2_read_register(MAG_SLAVE_ADDRESS, MAG_Y_H, &data[1]);
	g_mag.mag_y = (s16)(data[1] << 8 | data[0]);

	ist8310_iic2_read_register(MAG_SLAVE_ADDRESS, MAG_Z_L, &data[0]);
	ist8310_iic2_read_register(MAG_SLAVE_ADDRESS, MAG_Z_H, &data[1]);
	g_mag.mag_z = (s16)(data[1] << 8 | data[0]);

	ist8310_iic2_read_register(MAG_SLAVE_ADDRESS, TEMPERATURE_L, &data[0]);
	ist8310_iic2_read_register(MAG_SLAVE_ADDRESS, TEMPERATURE_H, &data[1]);
