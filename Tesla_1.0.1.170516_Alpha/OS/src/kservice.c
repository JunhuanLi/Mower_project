...
void rt_show_version(void)
{
	...
	sprintf((char*)lcd_id,"MOWER_LCD_DEV:%4X",lcd_dev.id);
	lcd_show_string(10,60,480,32,32,lcd_id);
	
	sprintf((char*)lcd_id," \\ | /");// \\其中一个是转义字符
	lcd_show_string(10,130,480,16,16,lcd_id);		//显示LCD ID	
	
	sprintf((char*)lcd_id,"- RT -     Thread Operating System");
	lcd_show_string(10,146,480,16,16,lcd_id);		//显示LCD ID	 
	
	sprintf((char*)lcd_id," / | \\     %d.%d.%d build %s",
			RT_VERSION, RT_SUBVERSION, RT_REVISION, __DATE__);
	lcd_show_string(10,162,480,16,16,lcd_id);		 

	sprintf((char*)lcd_id," 2006 - 2013 Copyright by rt-thread team");
	lcd_show_string(10,178,480,16,16,lcd_id);		 	
}
