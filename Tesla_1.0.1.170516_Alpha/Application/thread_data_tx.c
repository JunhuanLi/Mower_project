...
/*在分配堆栈空间时，必须要对齐*/
ALIGN(RT_ALIGN_SIZE)
char thread_datatx_stack[1024];
struct rt_thread thread_datatx;
void mower_data_tx_thread(void* parameter)
{
    ...
    while (1)
    {
		...
		*/
        ...
        sprintf(xstr_buffer, "data_tx %d", gps_idx++); 
        rt_kprintf("\r\n...__data_tx_thread %s  ",xstr_buffer);
		#endif
        //rt_thread_delay(20);// 1s 钟执行一次此线程
